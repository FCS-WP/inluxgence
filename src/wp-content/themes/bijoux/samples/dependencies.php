...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'WooCommerce', 'bijoux' ),
			'slug'     => 'woocommerce',
			'required' => true,
			'category' => 'required',
		),
		...
		array(
			'name'     => esc_html__( 'Elementor', 'bijoux' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html__( 'Vamtam Elementor Integration', 'bijoux' ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'bijoux' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'bijoux' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'bijoux' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'bijoux' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'bijoux' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'LiteSpeed Cache', 'bijoux' ),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'bijoux' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'bijoux' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'bijoux' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'bijoux' ),
		'description' => esc_html__( 'Description', 'bijoux' ),
		'status'      => esc_html__( 'Status', 'bijoux' ),
		'version'     => esc_html__( 'Version', 'bijoux' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'bijoux' );
	}
	if ( strpos( $plugin['slug'], 'bijoux' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'bijoux' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'bijoux' );
